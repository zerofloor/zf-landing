  ...
  return (
    <>
      {portfolio.map((portfolio_item, index) => {
        return <PortfolioCard key={index} portfolio_item={portfolio_item} />;
      })}
    </>
  );
};
...
const PortfolioCard = ({ portfolio_item }: { [key: string]: any }) => {
  return (
    <div className="flex-col w-11/12 sm:w-5/12 sm:flex-row border border-neutral-700 rounded-md  flex justify-between sm:items-center p-2 px-4 mb-2">
      <div className="flex flex-col ">
        <span className="text-xl leading-5 mt-3">
          {portfolio_item.properties.Name.title[0].plain_text}
        </span>
        <span className="text-sm text-gray-300">
          {portfolio_item.properties.Description.rich_text[0].plain_text}
        </span>
      </div>
      <div className="flex mt-2 sm:mt-0 gap-x-4">
        <a
          ...
          target="_blank"
          rel="noreferrer"
          href={portfolio_item.properties.Visit.url}
        >
